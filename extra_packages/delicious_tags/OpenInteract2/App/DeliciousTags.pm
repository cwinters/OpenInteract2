package OpenInteract2::App::DeliciousTags;

# $Id: DeliciousTags.pm,v 1.2 2005/03/18 03:34:57 lachoy Exp $

use strict;
use base qw( Exporter OpenInteract2::App );
use OpenInteract2::Manage;

$OpenInteract2::App::DeliciousTags::VERSION = sprintf("%d.%02d", q$Revision: 1.2 $ =~ /(\d+)\.(\d+)/);
@OpenInteract2::App::DeliciousTags::EXPORT  = qw( install );

my $NAME = 'delicious_tags';
...
OpenInteract2::App::DeliciousTags - Mark generic objects with simple text tags
...
=head1 SYNOPSIS
 ...
 # whenever it's saved or issues a 'post add' or 'post update'
 # notification; this is done automatically for you if you're using
 # the Common actions
...
The class L<OpenInteract2::DeliciousTaggableObject> is used for
getting data into and out of the system. If you mark your SPOPS object with:

 is_taggable = yes

then its methods will be available from every object in that
class. Otherwise you need to use the methods prefixed with 'c_'
(class).

=head2 Getting tags into the system

The best way is to use the action observer
L<OpenInteract2::Observer::AddDeliciousTags>. Once it's configured as
discussed in L<SYNOPSIS> it will sit back and watch all 'post add' and
'post update' action observations. When it catches one it will look
for the object added or updated and its asssociated tag data; if both
are found the observer will tag that object.
...
accomplish this in your action you might have:
 ...
 sub update {
     ...
     # ...if not so marked
     OpenInteract2::DeliciousTaggableObject::add_tags( $my_object, @tags );
 
     # ...or if $my_object is not an SPOPS object
     OpenInteract2::DeliciousTaggableObject->c_add_tags(
             'My Type',
             $my_object->id,
...
See L<OpenInteract2::DeliciousTag> for information on the additional methods.
...
L<OpenInteract2::DeliciousTagWatcher>
...
into a request to add L<OpenInteract2::DeliciousTaggableObject> to
